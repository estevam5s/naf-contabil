-- Schema simplificado para agendamentos de orientações fiscais
-- Sistema NAF Contábil - Fiscal Guidance Appointments (Versão Independente)
...
BEGIN
    LOOP
        ...
        new_protocol := 'FAP-' || TO_CHAR(NOW(), 'YYYYMMDD') || '-' || LPAD(FLOOR(RANDOM() * 10000)::TEXT, 4, '0');

        -- Verificar se o protocolo já existe
        SELECT EXISTS(SELECT 1 FROM fiscal_appointments WHERE fiscal_appointments.protocol = new_protocol) INTO protocol_exists;
